...
class BeerGame(gym.Env):
    ...
    def __init__(self, env_type: str, n_turns_per_game=20,
        ...
        self.inbound_shipments = []  # shipments inbound at each agent (for example, for the agent Distributor, how much the Manufacturer has shipped)
    ...
    def reset(self):
        ...
        if self.env_type == 'classical':
            temp_orders = [[4, 4]] * (self.n_agents - 1) + [[4]] # order lead time of 2 for all agents except the manufacturer
            ...
            if self.add_noise_initialization:
                ...
                inbound_shipments_noise = np.random.choice(np.arange(5), size=get_init_len(temp_inbound_shipments)) - 2
            ...
            if self.add_noise_initialization:
                ...
                inbound_shipments_noise = np.random.choice(np.arange(3), size=get_init_len(temp_inbound_shipments)) - 1
                temp_inbound_shipments = add_noise_to_init(temp_inbound_shipments, inbound_shipments_noise)
            ...
            if self.add_noise_initialization:
                ...
                inbound_shipments_noise = np.random.normal(loc=0, scale=5, size=get_init_len(temp_inbound_shipments))
                inbound_shipments_noise = np.clip(inbound_shipments_noise, -10, 10)  # clip to prevent negative inbound shipments
                temp_inbound_shipments = add_noise_to_init(temp_inbound_shipments, inbound_shipments_noise)
    ...
    def render(self, mode='human'):
        ...
        print('Stocks Levels (at the end of the turn): ', ", ".join([str(x) for x in self.stocks]))
